...
bool SynthSequencer::playSequence(std::string sequenceName, float startTime, bool isLooping) {
  //        synth().allNotesOff();
  ...
  if (sequenceName.size() > 0) {
    ...
    // Had to turn off the mutex lock because the callback is called immediately after
    // mEvents is empty, so lk.unlock() is not called and playSequence in loopSequence cannot
    // enter lock.
  ...
  if (isLooping) {
      // std::cout << "Adding sequence " << sequenceName << " " << isLooping << std::endl;
      std::function<void (std::string)> func = [this](std::string sequenceName){loopSequence(sequenceName);};
      registerSequenceEndCallback(func);
  } else {
      // mSequenceEndCallbacks.clear();
  }
  ...
  // std::cout << "Finished play function " << sequenceName << " " << isLooping << std::endl;

  return true;
}

void SynthSequencer::loopSequence(std::string sequenceName){
  // std::cout << "Looping " << sequenceName << std::endl;
  playSequence(sequenceName, 0.0f, true);
}
